...
def gpu_long_task(duration_minutes=5):
    ...
    if TORCH_AVAILABLE and torch is not None:
        ...
        if results["gpu_available"]:
            ...
            results["gpu_names"] = [
                torch.cuda.get_device_name(i) for i in range(results["gpu_count"])
            ]
            ...
            print(
                "   (Vous pouvez maintenant utiliser 'python -m slurmray' pour ouvrir le dashboard)"
            )
            ...
            while time.time() < end_time:
                ...
                if task_counter % 10 == 0:
                    ...
                    print(
                        f"⏱️  Temps écoulé: {elapsed/60:.1f} min | Restant: {remaining:.1f} min | Tâches: {task_counter}"
                    )
            ...
            print(
                f"⏳ Attente de {duration_minutes} minutes pour tester le dashboard..."
            )
...
def test_gpu_dashboard_long():
    ...
    try:
        result = cluster(
            gpu_long_task, args={"duration_minutes": 5}
        )  # 5 minutes pour explorer le dashboard
