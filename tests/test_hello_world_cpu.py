...
def test_hello_world_cpu():
    ...
    assert result["message"] == "Hello World from SLURM_RAY CPU!", f"Expected message, got: {result['message']}"
    ...
    assert result["cpu_count"] > 0, f"cpu_count should be > 0, got: {result['cpu_count']}"
