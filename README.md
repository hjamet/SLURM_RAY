...
| **Refactoring Architecture Strategy** | Restructurer le cœur de l'application `RayLauncher.py` en appliquant le Design Pattern "Strategy". Il s'agit de créer une classe abstraite `ClusterBackend` définissant l'interface commune (`submit`, `status`, `cancel`, `get_logs`), puis d'encapsuler toute la logique Slurm actuelle dans une classe concrète `SlurmBackend`. Cette étape est cruciale pour permettre l'ajout futur du backend "Desi" sans complexifier le code avec des conditions multiples. Le code existant doit être déplacé sans régression. | 📅 À faire | Corriger les incompatibilités avec Curnagl |
| **Implémentation Backend Desi** | Développer la classe `DesiBackend` pour le serveur `isipol09`. Ce backend doit gérer une exécution "Stateless" via SSH/SFTP. **Point clé :** Implémenter un système de **Scheduling Maison** ("Smart Lock") pour gérer la concurrence. Le runner doit vérifier un fichier de lock/ressources sur le serveur avant de se lancer. Si les ressources (GPU/CPU) sont occupées, il doit entrer dans une file d'attente active et afficher des logs clairs à l'utilisateur : "En attente... Position estimée X, Temps estimé Y". Une fois lancé, il utilise des ports Ray dynamiques pour une isolation parfaite. Le nettoyage (`rm -rf`) doit être garanti via des blocs `finally`. | 📅 À faire | Refactoring Architecture Strategy |
