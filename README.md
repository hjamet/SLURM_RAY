...
|-------|----------|------|-------------|
| **Implémentation Backend Desi** | Développer la classe `DesiBackend` pour le serveur `130.223.73.209` (isipol09). Ce backend doit gérer une exécution "Stateless" via SSH/SFTP. **Point clé :** Implémenter un système de **Scheduling Maison** ("Smart Lock") pour gérer la concurrence. Le runner doit vérifier un fichier de lock/ressources sur le serveur avant de se lancer. Si les ressources (GPU/CPU) sont occupées, il doit entrer dans une file d'attente active et afficher des logs clairs à l'utilisateur : "En attente... Position estimée X, Temps estimé Y". Une fois lancé, il utilise des ports Ray dynamiques pour une isolation parfaite. Le nettoyage (`rm -rf`) doit être garanti via des blocs `finally`. | 🏗️ En cours | - |
